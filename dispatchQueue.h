...
typedef struct dispatch_queue_item_t
    dispatch_queue_item_t; // the dispatch queue item type
typedef struct dispatch_queue_t dispatch_queue_t; // the dispatch queue type
typedef struct dispatch_queue_thread_t
    dispatch_queue_thread_t; // the dispatch queue thread type
...
struct dispatch_queue_thread_t {
    ...
    sem_t thread_semaphore; // the semaphore the thread waits on until a task is
                            // allocated
...
struct dispatch_queue_t {
    ...
    pthread_mutex_t
        queue_mutex;           // mutex used to synchronise access to the queue
    pthread_cond_t queue_cond; // condition variable used to signal events to
    ...
    volatile bool shutdown; // flag to indicate if the queue has been asked to
                            // shutdown (destroy)
    volatile bool waiting;  // flag to indicate if the queue is waiting for all
